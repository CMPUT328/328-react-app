...
import { listRegions, listTeams } from "../../graphql/queries";
...
const GlobalRanking = () => {
  ...
  async function fetchRegions() {
    try {
      const apiData = await API.graphql(graphqlOperation(listRegions));
      const regionsFromAPI = apiData.data.listRegions.items;
      const region_names = regionsFromAPI.map((region) => region.id);
      setRegions(region_names);
    } catch (error) {
      console.log(error);
    }
  }
  ...
  async function fetchTeams() {
    ...
    try {
      ...
      //   if team in teams have rank field null then set rank to 99999
      ...
      <div className="list-and-info-container">
        ...
        {selectedTeam && (
          <div className="team-details">
            ...
            <p className="team-info-rank">Rank: {selectedTeam.rank}</p>
            <p className="team-info-rank">Region: {selectedTeam.region}</p>
